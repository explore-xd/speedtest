package com.dss.speedtest.core.ping;
...
public abstract class PingStream {
    ...
    public PingStream(String server, String path, int pings, String errorHandlingMode, int connectTimeout, int soTimeout, int recvBuffer, int sendBuffer, Logger log) {
        this(server, path, pings, errorHandlingMode, connectTimeout, soTimeout, recvBuffer, sendBuffer, log, null);
    }

    public PingStream(String server, String path, int pings, String errorHandlingMode, int connectTimeout, int soTimeout, int recvBuffer, int sendBuffer, Logger log, String authorization) {
        this.server = server;
        this.path = path;
    ...
    private void init() {
        ...
        new Thread() {
            public void run() {
                ...
                try {
                    c = new Connection(server, connectTimeout, soTimeout, recvBuffer, sendBuffer, authorization);
                    if (stopASAP) {
                        try {
                            c.close();
                        } catch (Throwable t) {
    ...
    public abstract void onDone(int total, int lost);
