package com.athena.speedtest.core.worker;

import com.athena.speedtest.core.base.Connection;
import com.athena.speedtest.core.base.Utils;
import com.athena.speedtest.core.config.SpeedTestConfig;
import com.athena.speedtest.core.config.TelemetryConfig;
import com.athena.speedtest.core.download.DownloadStream;
import com.athena.speedtest.core.getIP.GetIP;
import com.athena.speedtest.core.log.Logger;
import com.athena.speedtest.core.ping.PingStream;
import com.athena.speedtest.core.serverSelector.TestPoint;
import com.athena.speedtest.core.telemetry.Telemetry;
import com.athena.speedtest.core.upload.UploadStream;
...
public abstract class SpeedTestWorker extends Thread {
    ...
    private void getIP() {
        if (getIPCalled) return;
        else getIPCalled = true;
    ...
    private void pingTest() {
        ...
        PingStream ps = new PingStream(backend.getServer(), backend.getPingURL(), config.getCount_ping(), config.getErrorHandlingMode(), config.getPing_connectTimeout(), config.getPing_soTimeout(), config.getPing_recvBuffer(), config.getPing_sendBuffer(), log, backend.getAuthorization()) {
            ...
            @Override
            public void onDone(int total, int lost) {
                onPingLostUpdate(total, lost);
            }
        };
        ps.join();
        if (stopASAP) return;
    ...
    public abstract void onPingLostUpdate(int total, int lost);
